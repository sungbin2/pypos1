...
@app.route('/goods/item_barcode', methods=['GET', ])
def _goods_item_barcode():
...
@app.route('/goods/item_barcode/<int:_id>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def _goods_item_barcode_(_id):
    store_id = c.session['store']
    gd = c.dict_itemgroup(orm, store_id)
    if c.is_json():
        if c.is_GET():
