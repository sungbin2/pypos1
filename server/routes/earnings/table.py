...
@app.route('/earnings/table', methods=['GET', ])
def _earnings_table():
...
@app.route('/earnings/table/<int:_id>&<dates>&<datee>', methods=['GET'])
def _earnings_table_(_id,dates,datee):
    store_id = c.session['store']
    cnt_days = {}
    cnt_days['상품목록'] = {}
    cnt_days['상품분류'] = {}
...
@app.route('/earnings/table1', methods=['GET', ])
def _earnings_table1():
    store_id = c.session['store']
    if c.is_GET():
...
@app.route('/earnings/table2', methods=['GET', ])
def _earnings_table2():
    store_id = c.session['store']
    if c.is_GET():
...
@app.route('/earnings/table3', methods=['GET', ])
def _earnings_table3():
    store_id = c.session['store']
    if c.is_GET():
...
@app.route('/earnings/table4', methods=['GET', ])
def _earnings_table4():
    store_id = c.session['store']
    if c.is_GET():
